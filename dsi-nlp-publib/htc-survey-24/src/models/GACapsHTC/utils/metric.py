...
def our_metrics(predictions, targets, tax_file: str, label2id: Dict[str, int]):
    # predictions are lists of lists, samples x label n (boolean)
    # Transform labels into required format for hierarchical metrics (samples, labels)

    pred_bin = np.array(predictions > .5, dtype=int)
    true_bin = np.array(targets, dtype=int)

    id2label = {v: k for k, v in label2id.items()}

    y_pred = [[id2label[lab] for lab in np.where(row == 1)[0]] for row in pred_bin]
    y_true = [[id2label[lab] for lab in np.where(row == 1)[0]] for row in true_bin]
    ...
    h_metrics = compute_hierarchical_metrics(y_true_bin, y_pred_bin, encoder_dump_or_mapping=mlb, taxonomy_path=Path(tax_file))
    return {**metrics, **h_metrics}
