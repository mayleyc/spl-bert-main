...
sys.path.append("/home/alessandro/work/repo/htc-survey")
sys.path.append("E:\\ricerca\\htc-survey")

from src.models.GACapsHTC.utils import logger
from src.models.GACapsHTC.model import classifier
from src.models.GACapsHTC.utils.configuration import Configuration
from src.models.GACapsHTC.utils.dataloader import HTCDataset, collate_fn
from src.models.GACapsHTC.utils.file_and_ckpt import *
from src.models.GACapsHTC.utils.loss import *
from src.models.GACapsHTC.utils.metric import *
...
def train(config):
    ...
    for epoch in range(epoch, config.training.num_epochs):
        ...
        with torch.no_grad():
            ...
            # our_performances = our_metrics(val_preds, val_labels, tax_file=config.testing.taxonomy_file, label2id=label_ids)
...
def test(config, best_setting=None):
    ...
    with torch.no_grad():
        ...
        our_performances = our_metrics(test_preds, test_labels, tax_file=config.testing.taxonomy_file, label2id=label_ids)
        ...
        pprint(our_performances)
        return our_performances
...
if __name__ == "__main__":
    ...
    logger.add_filehandler(config.path.log.filename)
    logger.logging_verbosity(config.path.log.level)
    ...
    perf = test(config, best_setting=best_setting)

    # Save performance metrics to file
    config_path = Path(config_path)
    res_path = config_path.parent.parent / "results"
    res_path.mkdir(parents=True, exist_ok=True)
    with open((res_path / config_path.stem).with_suffix(".json"), mode="w", encoding="utf-8") as res_o:
        json.dump(perf, res_o, indent=2)
