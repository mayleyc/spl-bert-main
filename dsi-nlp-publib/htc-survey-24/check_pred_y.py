...
bert_pred_y_fp = "dumps/BERT_SPL/bert_multilabel_BGC_concat_cls_3_SPL/run_2025-10-23_22-21-31/y_pred_repeat_1_2025-10-23_23-06-59.csv"
...
def main(): 
    ...
    # Get rows that are in violations but NOT in ME
    other_violations = viol_set - me_set
    ...
    print(f"HV rows: {np.array(viol_rows)+2}")
    # --- Count of samples by deepest positive level ---
    # Create a mapping from class index to level
    index_to_level = {}
    start_index = 0
    for level, num_nodes in enumerate(n_leaves, 1):
        end_index = start_index + num_nodes
        for i in range(start_index, end_index):
            index_to_level[i] = level
        start_index = end_index

    # For each sample, find the deepest level with a positive prediction
    deepest_levels = []
    for row in predictions:
        positive_indices = np.where(row == 1)[0]
        if len(positive_indices) == 0:
            deepest_levels.append(0)  # Level 0 for no predictions
        else:
            max_level = 0
            for index in positive_indices:
                level = index_to_level.get(index)
                if level and level > max_level:
                    max_level = level
            deepest_levels.append(max_level)

    # Count the occurrences of each deepest level
    level_counts = pd.Series(deepest_levels).value_counts().sort_index()

    print("\n--- Count of samples by deepest positive level ---")
    for level, count in level_counts.items():
        if level == 0:
            print(f"Samples with no positive predictions: {count}")
        else:
