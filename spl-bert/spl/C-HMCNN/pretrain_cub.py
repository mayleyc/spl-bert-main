...
class ConstrainedFFNNModel(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, hyperparams, dataset):
...
class ClassifierHead(nn.Module): # final layer for outputs
...
class Pretrain_Model(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, hyperparams, dataset):
        super().__init__()
        self.backbone = ConstrainedFFNNModel(input_dim, hidden_dim, output_dim, hyperparams, dataset)
        self.head = ClassifierHead(self.backbone.output_dim, output_dim = 373)
...
def main():
    ...
    model = Pretrain_Model(input_dims[data], hidden_dim, output_dim, hyperparams, args.dataset)
    ...
    criterion = nn.BCEWithLogitsLoss(reduction="none") #hierarchy = multilabel problem. reduction = none: 1 loss per example
    ...
    def evaluate_circuit(model, epoch, data_loader, data_split, prefix):
        total_loss = 0
        test_val_t = perf_counter()
        ...
        for i, (x,y) in enumerate(data_loader):
            ...
            bce_logits_loss = criterion(logits, y.float()).mean().item() # .mean() reduces the tensor to a scalar, .item() converts it to a float
            total_loss += bce_logits_loss

            pred_y = (logits > 0.5).float()       
        ...
        avg_bce_logits_loss = total_loss / (i+1)
        '''
        ...
        if "cub" in args.dataset:
            ...
            predicted_test = predicted_test.detach().to("cpu").numpy()
        ...
        print(f"BCE logits loss: {avg_bce_logits_loss}")
        ...
        return {
            ...
            f"{prefix}/bcell": (avg_bce_logits_loss, epoch, dt),
        }
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 0 and epoch != 0:
            ...
            for perf_name, (score, epoch, dt) in perf.items(): #perf_name = metric (accuracy, nll, etc.) Every 5 epochs
                ...
                if "valid" in perf_name and "bce_logits" in perf_name:
                    valid_loss = score
        ...
        if avg_loss < best_loss: #may change to valid_loss instead of avg_loss (training loss). avg_loss performs slightly better (longer training time)
            ...
            if args.exp_id:
                ...
                out_path_model = os.path.join(model_save_folder,  
                                              '{}_{}_{}_{}_nc{}_patience{}_pretrained_model.pth'.format(
                                                args.dataset, model.__class__.__name__, date_string, args.batch_size, args.no_constraints, patience_on
                                                ))
